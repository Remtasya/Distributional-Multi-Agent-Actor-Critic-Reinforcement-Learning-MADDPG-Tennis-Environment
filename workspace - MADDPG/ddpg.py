...
class DDPGAgent:
    def __init__(self, action_size, state_size, hidden_in_size, hidden_out_size, lr_actor, lr_critic, l2_decay = 0.0001, noise_type = 'OUNoise'):
        super(DDPGAgent, self).__init__()

        self.actor = Actor(action_size, state_size, hidden_in_size, hidden_out_size).to(device)
        self.critic = Critic(2*action_size, 2*state_size, hidden_in_size, hidden_out_size).to(device)
        self.target_actor =  Actor(action_size, state_size, hidden_in_size, hidden_out_size).to(device)
        self.target_critic = Critic(2*action_size, 2*state_size, hidden_in_size, hidden_out_size).to(device)
